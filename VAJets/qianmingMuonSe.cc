...
MuonIdSelector::MuonIdSelector(const edm::ParameterSet& iConfig)
    ...
    : moduleLabel_(iConfig.getParameter<std::string>("@module_label")), idLabel_(iConfig.existsAs<std::string>("idLabel") ? iConfig.getParameter<std::string>("idLabel") : "loose"), nTot_(0), nPassed_(0), MuonToken_(consumes<pat::MuonCollection>(iConfig.getParameter<edm::InputTag>("src"))), VertexToken_(consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("vertex")))
...
void MuonIdSelector::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    for (unsigned int iMu = 0; iMu < muons->size(); iMu++) {
        ...
        if (mu1.pt() > 20 && fabs(mu1.eta()) < 2.4 && mu1.isGlobalMuon() && mu1.isPFMuon() && (mu1.globalTrack()->normalizedChi2()) < 10 && (mu1.globalTrack()->hitPattern().numberOfValidMuonHits()) > 0 && (mu1.numberOfMatchedStations()) > 1 && fabs(d0vtx) < 0.2 && fabs(dzvtx) < 0.5 && /* (mu1.numberOfMatchedStations())>1 && */ (mu1.innerTrack()->hitPattern().numberOfValidPixelHits()) > 0 && (mu1.innerTrack()->hitPattern().trackerLayersWithMeasurement()) > 5 && fabs(isolation) < 0.15) {
            isTight = true;
        }

        //for fake muon study
        //  if(mu1.pt()>20 && fabs(mu1.eta())<2.4  && (mu1.isGlobalMuon() || mu1.isTrackerMuon()) && mu1.isPFMuon() && fabs(isolation)<3 && fabs(d0vtx)<0.2 && fabs(dzvtx)<0.5 && !((mu1.globalTrack()->normalizedChi2())<10 && (mu1.innerTrack()->hitPattern().trackerLayersWithMeasurement())>5 && fabs(isolation)<0.15)) { isTight = true;}  //&& (mu1.globalTrack()->hitPattern().numberOfValidMuonHits())>0  && (mu1.innerTrack()->hitPattern().numberOfValidPixelHits()) > 0 && (mu1.numberOfMatchedStations())>1

        if (mu1.pt() > 20 && fabs(mu1.eta()) < 2.4 && (mu1.isGlobalMuon() || mu1.isTrackerMuon()) && mu1.isPFMuon() && fabs(isolation) < 0.25) {
            isLoose = true;
        }
    ...
    for (unsigned int iMuon = 0; iMuon < muons->size(); iMuon++) {
        if (isPassing[iMuon])
            passingMuons->push_back(muons->at(iMuon));
    }
