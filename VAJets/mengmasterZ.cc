    ...
    for (const pat::Jet& jet : *jets_) {
        ...
        if (skipMuons_ && jet.muonMultiplicity() != 0) {
            for (const pat::Muon& muon : *muons_) {
                ...
                muonV.SetPtEtaPhiE(muon.p4().pt(), muon.p4().eta(), muon.p4().phi(), muon.p4().e());
                TLorentzVector jetV;
                jetV.SetPtEtaPhiE(jet.p4().pt(), jet.p4().eta(), jet.p4().phi(), jet.p4().e());
...
//------------------------------------
//------------------------------------
void ZPKUTreeMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
    using namespace edm;
    ...
    if (lep == 13) {
        ...
        for (int i = 0; i < 6; i++) {
            if (lep1_sign == genmuon_pid[i]) {
                ...
                if (dr_mugenmu < dr_temp) {
                    matchmuon_pt = genmuon_pt[i];
                    dr_temp      = dr_mugenmu;
                }
            }
        }
        if (dr_temp < 0.3)
            matchedgenMu1_pt = matchmuon_pt;
    }
    ...
    for (size_t ip = 0; ip < photons->size(); ip++) {
        ...
        if (passEleVetonew && fabs((*photons)[ip].eta()) < 1.4442 && (*photons)[ip].hadTowOverEm() < 0.0396 && photon_sieie[ip] < 0.01022 && chiso < 0.441 && nhiso < (2.725 + (0.0148 * (*photons)[ip].pt() + 0.000017 * (*photons)[ip].pt() * (*photons)[ip].pt())) && phoiso < (2.571 + 0.0047 * (*photons)[ip].pt())) {
            ismedium_photon = 1;
        }
        if (passEleVetonew && fabs((*photons)[ip].eta()) < 2.5 && fabs((*photons)[ip].eta()) > 1.566 && (*photons)[ip].hadTowOverEm() < 0.0219 && photon_sieie[ip] < 0.03001 && chiso < 0.442 && nhiso < (1.715 + (0.0163 * (*photons)[ip].pt() + 0.000014 * (*photons)[ip].pt() * (*photons)[ip].pt())) && phoiso < (3.863 + 0.0034 * (*photons)[ip].pt())) {
            ismedium_photon = 1;
        }
        ...
        ////Inverting loose ID
        if (passEleVetonew && fabs((*photons)[ip].eta()) < 1.4442 && (*photons)[ip].hadTowOverEm() < 0.0597 && chiso < std::min(0.2 * (*photons)[ip].pt(), 5. * 1.295) && nhiso < std::min(0.2 * (*photons)[ip].pt(), 5. * (10.910 + (0.0148 * (*photons)[ip].pt() + 0.000017 * (*photons)[ip].pt() * (*photons)[ip].pt()))) && phoiso < std::min(0.2 * (*photons)[ip].pt(), 5. * (3.630 + 0.0047 * (*photons)[ip].pt())) && !(photon_sieie[ip] < 0.01031 && chiso < 1.295 && nhiso < (10.910 + (0.0148 * (*photons)[ip].pt() + 0.000017 * (*photons)[ip].pt() * (*photons)[ip].pt())) && phoiso < (3.630 + 0.0047 * (*photons)[ip].pt()))) {
            ismedium_photon_f = 1;
        }
...
double ZPKUTreeMaker::getDR(double eta1, double phi1, double eta2, double phi2) {
    double DR = -1e1;
    if (fabs(phi1 - phi2) > 3.1415926)
        DR = sqrt((eta1 - eta2) * (eta1 - eta2) + (2 * 3.1415926 - fabs(phi1 - phi2)) * (2 * 3.1415926 - fabs(phi1 - phi2)));
    else
        DR = sqrt((eta1 - eta2) * (eta1 - eta2) + (fabs(phi1 - phi2)) * (fabs(phi1 - phi2)));
    return DR;
}
