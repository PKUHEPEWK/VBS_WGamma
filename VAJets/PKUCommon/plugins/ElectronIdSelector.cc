...
ElectronIdSelector::ElectronIdSelector(const edm::ParameterSet& iConfig)
    ...
    : moduleLabel_(iConfig.getParameter<std::string>("@module_label")), idLabel_(iConfig.existsAs<std::string>("idLabel") ? iConfig.getParameter<std::string>("idLabel") : "loose"), useDetectorIsolation_(iConfig.existsAs<bool>("useDetectorIsolation") ? iConfig.getParameter<bool>("useDetectorIsolation") : false), nTot_(0), nPassed_(0), ElectronToken_(consumes<pat::ElectronCollection>(iConfig.getParameter<edm::InputTag>("src"))), VertexToken_(consumes<reco::VertexCollection>(iConfig.getParameter<edm::InputTag>("vertex"))), RhoToken_(consumes<double>(iConfig.getParameter<edm::InputTag>("rho"))), effectiveAreas_((iConfig.getParameter<edm::FileInPath>("effAreasConfigFile")).fullPath()) {
    ...
    if ((idLabel_.compare("tight") == 0) || (idLabel_.compare("Tight") == 0) || (idLabel_.compare("TIGHT") == 0) || (idLabel_.compare("WP70") == 0) || (idLabel_.compare("wp70") == 0))
        applyTightID_ = true;
    else if ((idLabel_.compare("medium") == 0) || (idLabel_.compare("Medium") == 0) || (idLabel_.compare("MEDIUM") == 0) || (idLabel_.compare("WP80") == 0) || (idLabel_.compare("wp80") == 0))
        applyMediumID_ = true;
    else if ((idLabel_.compare("loose") == 0) || (idLabel_.compare("Loose") == 0) || (idLabel_.compare("LOOSE") == 0) || (idLabel_.compare("WP90") == 0) || (idLabel_.compare("wp90") == 0))
        applyLooseID_ = true;
    else if ((idLabel_.compare("veto") == 0) || (idLabel_.compare("Veto") == 0) || (idLabel_.compare("VETO") == 0) || (idLabel_.compare("VETOid") == 0) || (idLabel_.compare("VetoId") == 0))
        applyVetoID_ = true;
}
...
*/
        ...
        //2016 data
        isTight = (pt > 20.) && (!vtxFitConversion) && ((isEB && mHits <= 1 && isolation < 0.0588 && sigmaIEtaIEta < 0.00998 && dPhiIn < 0.0816 && dEtaIn < 0.00308 && hoe < 0.0414 && ooemoop < 0.0129 && fabs(d0vtx) < 0.05 && fabs(dzvtx) < 0.10) || (isEE && mHits <= 1 && isolation < 0.0571 && sigmaIEtaIEta < 0.0292 && dPhiIn < 0.0394 && dEtaIn < 0.00605 && hoe < 0.0641 && ooemoop < 0.0129 && fabs(d0vtx) < 0.10 && fabs(dzvtx) < 0.20));
        ...
        isMedium = (pt > 20.) && (!vtxFitConversion) && ((isEB && mHits <= 1 && isolation < 0.0695 && sigmaIEtaIEta < 0.00998 && dPhiIn < 0.103 && dEtaIn < 0.00311 && hoe < 0.253 && ooemoop < 0.134 && fabs(d0vtx) < 0.05 && fabs(dzvtx) < 0.10) || (isEE && mHits <= 1 && isolation < 0.0821 && sigmaIEtaIEta < 0.0298 && dPhiIn < 0.045 && dEtaIn < 0.00609 && hoe < 0.0878 && ooemoop < 0.13 && fabs(d0vtx) < 0.10 && fabs(dzvtx) < 0.20));

        isLoose = (pt > 20.) && (!vtxFitConversion) && ((isEB && mHits <= 1 && isolation < 0.0994 && sigmaIEtaIEta < 0.011 && dPhiIn < 0.222 && dEtaIn < 0.00477 && hoe < 0.298 && ooemoop < 0.241 && fabs(d0vtx) < 0.05 && fabs(dzvtx) < 0.10) || (isEE && mHits <= 1 && isolation < 0.107 && sigmaIEtaIEta < 0.0314 && dPhiIn < 0.213 && dEtaIn < 0.00868 && hoe < 0.101 && ooemoop < 0.14 && fabs(d0vtx) < 0.10 && fabs(dzvtx) < 0.20));

        isVeto = (pt > 20.) && (!vtxFitConversion) && ((isEB && mHits <= 2 && isolation < 0.175 && sigmaIEtaIEta < 0.0115 && dPhiIn < 0.228 && dEtaIn < 0.00749 && hoe < 0.356 && ooemoop < 0.299 && fabs(d0vtx) < 0.05 && fabs(dzvtx) < 0.10) || (isEE && mHits <= 3 && isolation < 0.159 && sigmaIEtaIEta < 0.037 && dPhiIn < 0.213 && dEtaIn < 0.00895 && hoe < 0.211 && ooemoop < 0.15 && fabs(d0vtx) < 0.10 && fabs(dzvtx) < 0.20));

        /// ------- Finally apply selection --------
        if (applyTightID_ && isTight)
            isPassing[iElec] = true;
        if (applyMediumID_ && isMedium)
            isPassing[iElec] = true;
        if (applyLooseID_ && isLoose)
            isPassing[iElec] = true;
        if (applyVetoID_ && isVeto)
            isPassing[iElec] = true;
    }

    for (unsigned int iElectron = 0; iElectron < electrons->size(); iElectron++) {
        if (isPassing[iElectron])
            passingElectrons->push_back(electrons->at(iElectron));
    }
